...
- [ ] Task 1: Configure Vercel Frontend Deployment (AC: 1)
  - [ ] Set up Vercel project and connect to GitHub repository with automatic deployments on main branch
  - [ ] Configure build settings: Next.js framework detection, build command 'turbo run build --filter=web'
  - [ ] Set up environment variables in Vercel dashboard for production API endpoints
  - [ ] Create vercel.json with specific PWA configuration:
```json
{
...
- [ ] Task 2: Configure Google Cloud Run Backend Deployment (AC: 2)
  - [ ] Create Dockerfile for Express.js app in apps/api/ with proper Node.js 20.11.1 base image
  - [ ] Create cloudbuild.yaml with exact configuration:
```yaml
steps:
...
```
  - [ ] Set up Cloud Build trigger connected to GitHub repository main branch
  - [ ] Configure Cloud Run service with specific resource allocation:
    - **CPU:** 1 vCPU, **Memory:** 2GB RAM
    - **Concurrency:** 80 requests per instance
    - **Startup timeout:** 240 seconds
    - **Request timeout:** 300 seconds
    - **Environment variables:** PORT=8080, NODE_ENV=production
  - [ ] Set up Cloud Run IAM permissions for service account access
- [ ] Task 3: Environment Management & Secrets (AC: 3)
  - [ ] Create GCP Service Account for deployment with exact IAM roles:
  ...
  - [ ] Securely store the Service Account JSON key as GitHub Actions Secret named `GCP_SERVICE_ACCOUNT_KEY`
  - [ ] Set up Google Secret Manager for production secrets with these keys:
...
- [ ] Task 4: Health Check Verification (AC: 4)
  - [ ] Add post-deployment verification step in GitHub Actions workflow
  - [ ] Create health check script that curls both Vercel frontend and Cloud Run backend /health endpoints
  - [ ] Configure timeout and retry logic for health check verification
  - [ ] Add deployment status notification on health check success/failure
- [ ] Task 5: Deployment Pipeline Robustness (AC: 5, 6)
  - [ ] Create GitHub Actions workflow (.github/workflows/deploy.yml) with complete pipeline:
```yaml
name: Deploy
on:
  push:
...
```
  - [ ] Configure Cloud Build trigger for automatic container builds on GitHub push
  - [ ] Implement Cloud Run revision-based rollback with specific commands:
  ...
  - [ ] Create rollback script (scripts/rollback.sh) with error handling:
  ...
  - [ ] Set up deployment failure detection triggers:
